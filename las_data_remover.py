...
def create_laz_file(laz_file_path, values, epsg_value, point_format=8, version="1.4", include_confidence=False, remove_data=None):
    """
    ...
    :param remove_data: List of data types to remove (e.g., ['nir', 'rgb']).
    """
    ...
    if "rgb" in values and "rgb" not in remove_data:
    ...
    if "infrared" in values and len(values["infrared"]) > 0 and "nir" not in remove_data:
